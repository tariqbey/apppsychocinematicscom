...
export const DefiniteChiefAimCard = ({ aim, onEdit }: DefiniteChiefAimCardProps) => {
  ...
  return (
    <div 
      className={`glass-card p-4 sm:p-6 cinematic-border relative overflow-hidden group transition-all duration-500 hover:border-gold/40 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      style={{ 
        boxShadow: '0 0 25px rgba(212, 175, 55, 0.08), inset 0 0 40px rgba(212, 175, 55, 0.03)',
      }}
    >
      {/* Animated decorative elements */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-gold/10 to-transparent rounded-bl-full transition-opacity duration-500 group-hover:opacity-70" />
      <Sparkles className="absolute top-4 right-16 w-3 h-3 text-gold/30 animate-pulse" />
      
      <div className="flex items-center justify-between mb-4 sm:mb-6 relative">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br from-gold/20 to-gold/5 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
        ...
        {onEdit && (
          <Button
            ...
            size="sm"
            onClick={onEdit}
            className="gap-2 text-muted-foreground hover:text-gold text-xs sm:text-sm"
          >
      ...
      <div className="space-y-3 sm:space-y-4 relative">
        {hasAim ? (
          <>
            <div className="p-3 sm:p-4 rounded-lg bg-secondary/50 border-l-2 border-gold transition-all duration-300 hover:bg-secondary/70">
            ...
            <div className="p-3 sm:p-4 rounded-lg bg-gradient-to-br from-gold/5 to-transparent border border-gold/20 transition-all duration-300 hover:border-gold/40">
              <div className="flex items-center gap-2 mb-1">
          ...
          <div className="p-4 sm:p-6 rounded-lg bg-secondary/30 border border-dashed border-gold/30 text-center transition-all duration-300 hover:border-gold/50">
