  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = ringCount; i >= 0; i--) {
        const ringRadius =
          baseRadius + 20 + i * 25 + Math.sin(phaseRef.current + i) * 5;
        const alpha = (0.15 - i * 0.04) * (currentState === "speaking" ? 1.5 : 1);
      ...
      const pulseScale =
        currentState === "idle"
          ...
          : currentState === "speaking"
            ? 1 + currentAudioLevel * 0.3 + Math.sin(phaseRef.current * 2) * 0.05
            : currentState === "listening"
              ? 1 + currentAudioLevel * 0.2 + Math.sin(phaseRef.current * 3) * 0.03
      ...
      ctx.shadowColor =
        currentState === "speaking"
          ...
          : currentState === "listening"
            ...
            : "hsl(43, 60%, 40%)";
      ctx.shadowBlur =
        currentState === "speaking" ? 40 + currentAudioLevel * 30 : 20;
      ctx.beginPath();
      ctx.arc(centerX, centerY, currentRadius, 0, Math.PI * 2);
